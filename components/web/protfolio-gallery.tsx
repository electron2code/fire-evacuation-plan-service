...
export default function PortfolioGallery() {
    ...
    useEffect(() => {
        const fetchPortfolioImages = async () => {
            ...
            try {
                ...
                console.log("Something went wrong when fetching gallery");
            } finally {
                setIsLoading(false);
            }
        }
    ...
    if (isLoading) {
        return (
            <div className="w-full min-h-screen relative">
                <div className="pointer-events-none absolute z-5 left-1/2 top-1/2 -translate-1/2" role="status">
    ...
    if (!portfolioImages.length) {
        return (
            <div className="w-full px-4 md:px-6 flex justify-center items-center min-h-screen relative">
                <span className="text-gray-400 text-2xl md:text-3xl">No gallery Items</span>
            </div>
        )
    }
    return (
