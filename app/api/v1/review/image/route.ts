...
export async function GET(request: Request) {
    try {
        ...
        if (!outsideReviewImages.length) {
            return NextResponse.json({ message: "No more outside review images" }, { status: 404 });
        }
        ...
        for (const image of outsideReviewImages) {
...
export async function DELETE(request: Request) {
    try {
        ...
        const deletedOutsideReviewImage = await prisma.outsideReviewImage.delete({
            where: { id, key }
        });

        if (!Object.keys(deletedOutsideReviewImage).length) {
